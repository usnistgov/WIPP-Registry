...
SAML2_AUTH = {
    ...
    'CREATE_USER': os.environ["SAML_CREATE_USER"], # Create a new Django user when a new user logs in. Defaults to True.
    'ATTRIBUTES_MAP': {  # Change to corresponding SAML2 userprofile attributes. Here an example is provided for Keycloak, with email, first_name and last_name added from built-in Mappers in the client, and username as a custom Mapper.
        'email': os.environ["SAML_ATTRIBUTES_MAP_EMAIL"],
        'username': os.environ["SAML_ATTRIBUTES_MAP_USERNAME"],
        'first_name': os.environ["SAML_ATTRIBUTES_MAP_FIRSTNAME"],
        'last_name': os.environ["SAML_ATTRIBUTES_MAP_LASTNAME"],
    },
    'ASSERTION_URL': os.environ["SAML_ASSERTION_URL"], # Custom URL to validate incoming SAML requests against
    'ENTITY_ID': os.environ["SAML_ENTITY_ID"], # Populates the Issuer element in authn request
    'NAME_ID_FORMAT': os.environ["SAML_ENTITY_ID"], # Sets the Format property of authn NameIDPolicy element
    'USE_JWT': os.environ["SAML_USE_JWT"],
    'SAML_CLIENT_SETTINGS': os.environ["SAML_CLIENT_SETTINGS"]
}